...
from reCBZ2 import wrappers, util
from reCBZ2.profiles import ProfileDict
...
def unix_like_glob(arg_list: list) -> list:
    """
    ...
    :param list arg_list: List of arguments to flatten
    :return: List of flattened args
    :rtype: list
    """
    import glob
    ...
    if args.show_config:
        for section in config._cfg.items():
            ...
            for key, val in section[1].items():
                ...
                print(
                    f"{key} =".ljust(18), f"'{modified}'".ljust(8), f"(default '{val}')"
                )
